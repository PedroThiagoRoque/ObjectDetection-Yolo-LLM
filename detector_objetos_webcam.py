...
def stream_video(video_path: str, model_path: str, summary_interval: int = 5):
    """Generator que produz (frame RGB numpy, texto) em tempo real."""
    load_dotenv()
    ...
    while True:
        ...
        # Detecção YOLO (para overlay)
        results = model(frame, verbose=False)
        annotated = results[0].plot()  # ainda BGR
...
def build_interface():
    with gr.Blocks(title="Detector de Objetos (YOLO + GPT‑4o Visão) – Streaming") as demo:
        gr.Markdown("## Detecção em Tempo Real + Resumo Multimodal GPT‑4o-mini")
        gr.Markdown(
            "Envie um vídeo; cada frame será analisado pelo YOLO e pelo modelo GPT‑4o para gerar uma descrição curta em tempo real."
        )
